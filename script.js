...
function wireContactForm(){
  ...
  form.addEventListener("submit", async (e)=>{
    ...
    // Optional: platform selection for marketing preview
    // Add platform selection inputs in contact.html (see instructions below)
    const platformInput = form.querySelector('input[name="platform"]:checked');
    const platform = platformInput ? platformInput.value : 'email';
    const platformCheckboxes = Array.from(form.querySelectorAll('input[name="socialPlatforms"]:checked')).map(n=>n.value);
    ...
    try {
      const res = await fetch('/api/messages', {
        ...
        body: JSON.stringify({
          ...
          platform, platformsSelected: platformCheckboxes
        })
      });
      const json = await res.json();
      if(!res.ok || !json.success) {
        out.textContent = 'Server error: could not save message.';
        out.style.color = 'crimson';
        return;
      }
      ...
      // store last message to session so the mock pages can render it
      sessionStorage.setItem('lastMessage', JSON.stringify(json.message));

      // redirect to mock preview (email or social). If social, pick first selected or 'facebook'
      if(platform === 'email'){
        window.location.href = '/mock_email.html';
      } else {
        const first = (platformCheckboxes.length && platformCheckboxes[0]) || 'facebook';
        window.location.href = '/mock_social.html?platform=' + encodeURIComponent(first);
      }

    } catch(err){
      console.error(err);
      out.textContent = 'Network error: cannot reach server.';
      out.style.color = 'crimson';
    }
  });
}
